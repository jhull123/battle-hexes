...
- **REQ-009**: Return combat events newest first. Preserve that ordering in the Game Log panel. Within a turn/player record, render combat events above reinforcement events because combat occurs later.
...
- **REQ-012**: The block is collapsed by default and uses a bold, interactive `Combat - <CRT outcome text>` heading. Expanding it reveals the combat details, beginning with labeled `Attacking` and `Defending` participant lines.
...
- **REQ-016**: Show the CRT outcome only in the combat heading. Do not repeat it in a `Result` line or render the player-readable result summary below the details.
- **REQ-017**: When nonempty, render `Eliminated: <unit names>` in the expanded details. Do not render a `Retreated` line or retreat destination coordinates.
- **REQ-018**: Do not render an empty eliminated label.
...
- **AC-004**: Given a defender retreat result where the defender successfully retreats, when the event is returned, then it contains the CRT outcome, a player-readable summary, the defender in `retreatedUnits`, and no retreat destination coordinate; when rendered, the outcome appears in the collapsed heading without a repeated summary or Retreated line.
...
- **AC-006**: Given a combat has no eliminations, when rendered, then no Eliminated line is displayed; Retreated lines are never displayed.
- **AC-007**: Given several normal combats resolve for one player in one turn, when the Game Log is returned and rendered, then they appear as separate combat blocks in newest-first order above any reinforcement events under that player’s one turn heading.
...
- **Frontend unit tests**: Verify collapsed combat-block organization and headings, expansion behavior, small stat styling, odds and die-roll lines, terrain explanation, omitted repeated result/retreat details, optional elimination lines, combat-before-reinforcement ordering, turn/player grouping, and refresh rendering.
...
```text
**Combat - Defender Retreat 2 Hexes** (collapsed by default)
...
The stat triplets use smaller text in the UI. The result and retreat summary are not repeated in the expanded content. No result event is recorded for a combat candidate skipped because a participant was already removed before it could resolve.
